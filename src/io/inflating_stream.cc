...
namespace bsw::io {
	...
	struct inflating_stream_buf::impl {
		...
		impl (std::istream& istr, decompression_stream* cstr)
			: m_istr (&istr),
			  m_ostr (nullptr),
			  m_eof (false),
			  m_buffer (INFLATE_BUFFER_SIZE),
		...
		impl (std::ostream& ostr, decompression_stream* cstr)
			: m_istr (nullptr),
			  m_ostr (&ostr),
			  m_eof (false),
			  m_buffer (INFLATE_BUFFER_SIZE),
	...
	inflating_stream_buf::inflating_stream_buf (std::istream& istr, decompression_stream* dstr)
		:
		buffered_stream_buf (STREAM_BUFFER_SIZE, std::ios::in),
		m_pimpl (spimpl::make_unique_impl<inflating_stream_buf::impl> (istr, dstr)) {
	...
	inflating_stream_buf::inflating_stream_buf (std::ostream& ostr, decompression_stream* dstr)
		:
		buffered_stream_buf (STREAM_BUFFER_SIZE, std::ios::out),
		m_pimpl (spimpl::make_unique_impl<inflating_stream_buf::impl> (ostr, dstr)) {
	}
...
#include <bsw/warn/pop.hh>
	...
	int inflating_stream_buf::write_to_device (const char* buffer, std::streamsize length) {
		...
		for (;;) {
			...
			if (rc == compression_stream::STREAM_END) {
				m_pimpl->m_ostr
					   ->write (m_pimpl->m_buffer.data (), INFLATE_BUFFER_SIZE - m_pimpl->m_decompressor->avail_out ());
	...
	inflating_ios::inflating_ios (std::ostream& ostr, decompression_stream* dstr)
		:
		_buf (ostr, dstr) {
		bsw_ios_init(&_buf);
	}
	...
	inflating_ios::inflating_ios (std::istream& istr, decompression_stream* dstr)
		:
		_buf (istr, dstr) {
		bsw_ios_init(&_buf);
	}
	...
	inflating_output_stream::inflating_output_stream (std::ostream& ostr, decompression_stream* dstr)
		:
		std::ostream (&_buf),
		inflating_ios (ostr, dstr) {
	}
	...
	inflating_input_stream::inflating_input_stream (std::istream& istr, decompression_stream* dstr)
		:
		std::istream (&_buf),
		inflating_ios (istr, dstr) {
	}
