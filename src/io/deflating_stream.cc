...
#include <bsw/io/deflating_stream.hh>
...
namespace bsw::io {
	...
	struct deflating_stream_buf::impl {
		...
		impl (std::istream& istr, compression_stream* cstr)
			: m_istr (&istr),
			  m_ostr (nullptr),
			  m_buffer (DEFLATE_BUFFER_SIZE),
			  m_eof (false),
		...
		impl (std::ostream& ostr, compression_stream* cstr)
			: m_istr (nullptr),
			  m_ostr (&ostr),
			  m_buffer (DEFLATE_BUFFER_SIZE),
			  m_eof (false),
	...
	deflating_stream_buf::deflating_stream_buf (std::istream& istr, compression_stream* cstr)
		: buffered_stream_buf (STREAM_BUFFER_SIZE, std::ios::in),
		  m_pimpl (spimpl::make_unique_impl<deflating_stream_buf::impl> (istr, cstr)) {
	...
	deflating_stream_buf::deflating_stream_buf (std::ostream& ostr, compression_stream* cstr)
		:
		buffered_stream_buf (STREAM_BUFFER_SIZE, std::ios::out),
		m_pimpl (spimpl::make_unique_impl<deflating_stream_buf::impl> (ostr, cstr)) {
	...
	int deflating_stream_buf::close () {
		...
		if (m_pimpl->m_ostr) {
			if (m_pimpl->m_compressor->next_out ()) {
				...
				m_pimpl->m_ostr
					   ->write (m_pimpl->m_buffer.data (), DEFLATE_BUFFER_SIZE - m_pimpl->m_compressor->avail_out ());
				...
				while (rc != compression_stream::STREAM_END) {
					...
					m_pimpl->m_ostr->write (m_pimpl->m_buffer.data (),
											DEFLATE_BUFFER_SIZE - m_pimpl->m_compressor->avail_out ());
					if (!m_pimpl->m_ostr->good ()) {
						RAISE_EX("Failed writing deflated data to output stream");
					}
					m_pimpl->m_compressor->set_next_out ((unsigned char*)m_pimpl->m_buffer.data ());
...
#include <bsw/warn/pop.hh>
	...
	deflating_ios::deflating_ios (std::ostream& ostr, compression_stream* cstr)
		:
		m_buf (ostr, cstr) {
		bsw_ios_init(&m_buf);
	}
	...
	deflating_ios::deflating_ios (std::istream& istr, compression_stream* cstr)
		:
		m_buf (istr, cstr) {
		bsw_ios_init(&m_buf);
	}
	...
	deflating_output_stream::deflating_output_stream (std::ostream& ostr, compression_stream* cstr)
		:
		std::ostream (&m_buf),
		deflating_ios (ostr, cstr) {
	}
	...
	deflating_input_stream::deflating_input_stream (std::istream& istr, compression_stream* cstr) :
		std::istream (&m_buf),
		deflating_ios (istr, cstr) {
	}
