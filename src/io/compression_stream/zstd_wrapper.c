...
int zstd_deflate (z_streamp strm, int flush) {
  ...
  if (zwc->zbc == NULL) {
    ...
    if (zwc->zbc == NULL)
      return ZWRAPC_finishWithError (zwc, strm, 0);
    {
      int const initErr = ZWRAP_initializeCStream (zwc, NULL, 0, (flush == Z_FINISH) ? strm->avail_in
    ...
    if (zwc->totalInBytes == 0) {
      if (zwc->comprState == ZWRAP_useReset) {
        ...
        int const res = ZWRAP_initializeCStream (zwc, NULL, 0, (flush == Z_FINISH) ? strm->avail_in
                                                                                   : ZSTD_CONTENTSIZE_UNKNOWN);
        if (res != Z_OK)
          return ZWRAPC_finishWithError (zwc, strm, res);
        if (flush != Z_FINISH)
...
int zstd_inflate_sync (z_streamp strm) {
  ...
  return zstd_inflate (strm, Z_INFLATE_SYNC);
}
...
int zstd_compress (Bytef* dest, uLongf* destLen, const Bytef* source, uLong sourceLen) {
  {
    ...
    LOG_WRAPPERD("zstd_compress sourceLen=%d dstCapacity=%d\n",
