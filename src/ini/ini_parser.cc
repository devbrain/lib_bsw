...
#include <bsw/ini/config_parser.hh>

#include <ryml_std.hpp>
#include <ryml.hpp>

#include "../thirdparty/inih.h"
      ...
      class json_builder {
        public:
          void add (const std::string& section, const std::string& key, const std::string& value) {
            ...
            double fval;
            bool bval;
            if (number_parser::try_parse (value, ival)) {
              add_internal (section, key, ival);
            } else if (number_parser::try_parse_float (key, fval)) {
              add_internal (section, key, (float)fval);
            } else if (number_parser::try_parse_bool (key, bval)) {
