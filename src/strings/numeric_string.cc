...
#include <bsw/strings/numeric_string.hh>
#include <bsw/exception.hh>
#include <bsw/strings/string_utils.hh>
#include <fpe/double-conversion/double-conversion.h>
#include <fpe/double-conversion/ieee.h>
...
namespace bsw {
  ...
  void float_to_str (char* buffer, int bufferSize, float value, int lowDec, int highDec) {
    ...
    DoubleToStringConverter dc (flags, BSW_FLT_INF, BSW_FLT_NAN, BSW_FLT_EXP, lowDec, highDec, 0, 0);
  ...
  void float_to_fixed_str (char* buffer, int bufferSize, float value, int precision) {
    ...
    int flags = DoubleToStringConverter::UNIQUE_ZERO |
                DoubleToStringConverter::EMIT_POSITIVE_EXPONENT_SIGN;
    DoubleToStringConverter dc (flags, BSW_FLT_INF, BSW_FLT_NAN, BSW_FLT_EXP,
  ...
  std::string& float_to_str (std::string& str, float value, int precision, int width, char thSep, char decSep) {
    ...
    char buffer[BSW_MAX_FLT_STRING_LEN];
    float_to_str (buffer, BSW_MAX_FLT_STRING_LEN, value);
    str = buffer;
  ...
  std::string& float_to_fixed_str (std::string& str, float value, int precision, int width, char thSep, char decSep) {
    ...
    char buffer[BSW_MAX_FLT_STRING_LEN];
    float_to_fixed_str (buffer, BSW_MAX_FLT_STRING_LEN, value, precision);
    str = buffer;
  ...
  std::string& double_to_str (std::string& str, double value, int precision, int width, char thSep, char decSep) {
    ...
    char buffer[BSW_MAX_FLT_STRING_LEN];
    double_to_str (buffer, BSW_MAX_FLT_STRING_LEN, value);
  ...
  std::string& double_to_fixed_str (std::string& str, double value, int precision, int width, char thSep, char decSep) {
    ...
    char buffer[BSW_MAX_FLT_STRING_LEN];
    double_to_fixed_str (buffer, BSW_MAX_FLT_STRING_LEN, value, precision);
