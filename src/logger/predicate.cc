...
namespace bsw::logger {
	...
	not_predicate_c::not_predicate_c (predicate_ptr_t operand)
	...
	or_predicate_c::or_predicate_c (predicate_ptr_t operand1,
									predicate_ptr_t operand2)
		: m_operand1 (std::move (operand1)),
		  m_operand2 (std::move (operand2)) {
	}

	bool or_predicate_c::call (const record_ptr_t record) const {
		bool r1 = !m_operand1 || m_operand1->call (record);
		if (r1) {
			return true;
		}
		return !m_operand2 || m_operand2->call (record);
	}

	and_predicate_c::and_predicate_c (predicate_ptr_t operand1,
									  predicate_ptr_t operand2)
		: m_operand1 (std::move (operand1)),
		  m_operand2 (std::move (operand2)) {
	}
	...
	if_predicate_c::if_predicate_c (predicate_ptr_t condition,
									predicate_ptr_t yesBranch,
									predicate_ptr_t noBranch)
		: m_condition (std::move (condition)),
		  m_yesBranch (std::move (yesBranch)),
		  m_noBranch (std::move (noBranch)) {
	}
	...
	file_name_eq_c::file_name_eq_c (std::string fileName)
