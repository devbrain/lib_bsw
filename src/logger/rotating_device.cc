    ...
    namespace priv
    {
        ...
        public:
            rotating_device_c(formatter_ptr_t formatter, predicate_ptr_t predicate,
                const bsw::filesystem::path& prefix, unsigned num, std::size_t max_lines);
        private:
        ...
        private:
            struct file_name_s
            {
                file_name_s(unsigned  n_, const bsw::filesystem::path& name_)
                    : n(n_),
                    name(name_)
                {
...
#include "bsw/warn/pop.hh"
        ...
        bool rotating_device_c::_open_device()
        {
            ...
            for (const auto& entry : bsw::filesystem::directory_iterator(m_path))
            {
                bsw::filesystem::path path = entry.path();
                if (!bsw::filesystem::is_directory(path))
                {
                    ...
                    if (idx == 0)
                    {
                        ...
                        if (n > -1)
                        {
                            candidates.push_back(file_name_s(static_cast<unsigned>(n), obj_name));
                        }
                    }
                }
            }
        ...
        bool rotating_device_c::_start_pool()
        {
            ...
            auto new_fname = m_path / bsw::filesystem::path(os.str());
            ...
            if (device)
            {
                ...
                if (log_seq.name.empty())
                {
                    ...
                    if (bsw::filesystem::exists(old_fname) && !bsw::filesystem::is_directory(old_fname))
                    {
                        bsw::filesystem::remove(old_fname);
                    }
                }
    ...
    // ----------------------------------------------------------
    device_ptr_t rotating_device(const bsw::filesystem::path& prefix, unsigned num, std::size_t max_lines, predicate_ptr_t predicate)
    {
        ...
        device_ptr_t dev(new priv::rotating_device_c(fmatter, predicate, prefix, num, max_lines));
        return dev;
    }
    // ----------------------------------------------------------
    device_ptr_t rotating_device(const bsw::filesystem::path& prefix, unsigned num, std::size_t max_lines)
    {
