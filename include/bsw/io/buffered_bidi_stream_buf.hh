...
#ifndef BSW_ARCHIVE_BUFFERED_BIDI_STREAM_BUF_HH
#define BSW_ARCHIVE_BUFFERED_BIDI_STREAM_BUF_HH
...
namespace bsw::io {
	...
	class BSW_EXPORT basic_buffered_bidi_stream_buf : public std::basic_streambuf<ch, tr>
		...
		/// for implementing an iostream.
	{
	 ...
	 public:
		basic_buffered_bidi_stream_buf (std::streamsize bufferSize, openmode mode)
			:
			_bufsize (bufferSize),
		...
		basic_buffered_bidi_stream_buf (const basic_buffered_bidi_stream_buf&) = delete;
		basic_buffered_bidi_stream_buf& operator= (const basic_buffered_bidi_stream_buf&) = delete;

		~basic_buffered_bidi_stream_buf () {
			delete[] _pReadBuffer;
		...
		virtual int_type overflow (int_type c) {
			...
			if (flush_buffer () == std::streamsize (-1)) {
				return char_traits::eof ();
			}
